...
export interface ImportSummary {
  ...
  errors: string[];
  orders: { po: string; smartsheetPo: string; status: string; customer: string }[];
}
...
export class SmartsheetImportService {
  ...
  async importMayOrders(sheetId: string, from = '2026-05-15', to = '2026-05-31'): Promise<ImportSummary> {
    const summary: ImportSummary = {
      ordersCreated: 0, ordersSkipped: 0, customersCreated: 0,
      attachmentsImported: 0, commentsImported: 0, errors: [], orders: [],
    };

    // Fetch full sheet with attachments + discussions
    this.logger.log('Fetching Smartsheet...');
    ...
    for (const row of rows) {
      try {
        ...
        summary.orders.push({
          ...
          smartsheetPo,
          status: rawStatus,
          customer: storeName || fullName,
        });

        // ── Attachments: customer-uploaded → Reference image, others → Design file ──
        const attachments: any[] = row.attachments || [];
        for (const att of attachments) {
          const dl = await this.downloadAttachment(sheetId, att.id);
          if (!dl) continue;
          const attEmail = (att.createdBy?.email || '').toLowerCase().trim();
          const isCustomer = attEmail && attEmail === email.toLowerCase().trim();
          await this.cadRepo.save(this.cadRepo.create({
            ...
            revisionNumber: 1,
            designerNotes: isCustomer ? 'Reference image' : 'Smartsheet import',
    ...
    this.logger.log(`Import complete — created ${summary.ordersCreated}, skipped ${summary.ordersSkipped}, errors ${summary.errors.length}`);
    return summary;
  }

  // ── Second pass: attach images + comments to already-imported orders ──
  async patchMediaAndComments(sheetId: string, from = '2026-05-15', to = '2026-05-31'): Promise<{ attachmentsImported: number; commentsImported: number; errors: string[] }> {
    ...
    for (const row of rows) {
      try {
        ...
        // Attachments: customer-uploaded → Reference image, others → Design file
        const customerEmail = (getCell(row, 'Email (final)') || getCell(row, 'Email') || '').toLowerCase().trim();
        for (const att of (row.attachments || [])) {
          ...
          if (!dl) continue;
          const attEmail = (att.createdBy?.email || '').toLowerCase().trim();
          const isCustomer = attEmail && attEmail === customerEmail;
          await this.cadRepo.save(this.cadRepo.create({
            orderId: order.id,
            originalName: dl.originalName,
            fileName: dl.fileName,
