...
export class SmartsheetWebhookService {
  ...
  // Cached title→columnId map; invalidated after 10 minutes
  ...
  private async getColMap(): Promise<Record<string, number>> {
    if (this.colCache && Date.now() - this.colCacheAt < this.COL_CACHE_TTL) {
  ...
  // ── Register a new Smartsheet webhook ────────────────────────────────────
  async registerWebhook(): Promise<{ webhookId: number; callbackUrl: string; status: string }> {
    const backendUrl = this.config.get('BACKEND_PUBLIC_URL', 'http://localhost:4000');
    const callbackUrl = `${backendUrl}/smartsheet/webhook/callback`;
    ...
    const data = await this.smFetch('/webhooks', {
      ...
      body: JSON.stringify({
        ...
        events: [{ objectType: 'row', eventType: 'created_or_updated' }],
        version: 1,
      }),
    });
  ...
  // ── Handle incoming Smartsheet webhook payload ────────────────────────────
  async processWebhookEvent(payload: any): Promise<{ processed: number; skipped: number }> {
    ...
    const rowIds = [...new Set(
      events
        .filter(e => e.objectType === 'row' && (e.eventType === 'updated' || e.eventType === 'created'))
        .map(e => String(e.id)),
    )];

    this.logger.log(`Webhook received — ${events.length} events, ${rowIds.length} unique rows`);

    for (const rowId of rowIds) {
      try {
        const synced = await this.syncRow(rowId);
        if (synced) result.processed++; else result.skipped++;
      } catch (e) {
  ...
  // ── Sync a single Smartsheet row to its matching order ───────────────────
  private async syncRow(rowId: string): Promise<boolean> {
    ...
    // Try matching by smartsheetRowId first (faster), then by refCustomerPo
    ...
    this.logger.log(
      `Synced order ${order.poNumber} ← rowId ${rowId}` +
      (newStatus ? ` → status: ${newStatus}` : ''),
    );
    return true;
  }

  // ── Manual full-sheet sync (re-sync all imported orders) ─────────────────
  async syncAll(from?: string, to?: string): Promise<{ synced: number; skipped: number; unmapped: string[]; errors: string[] }> {
    ...
    // Refresh cache while we have the full sheet
    ...
    for (const row of rows) {
      try {
        ...
        if (newStatus) {
          ...
          result.unmapped.push(`${smartsheetPo}: "${rawStatus}"`);
        }
    ...
    this.logger.log(`Full sync complete — synced:${result.synced} skipped:${result.skipped} unmapped:${result.unmapped.length} errors:${result.errors.length}`);
    return result;
  }
}
