...
export default function Dashboard() {
  ...
  const OrderRow = ({ o }: { o: any }) => {
    ...
    return (
      <div className="order-row-card" onClick={() => router.push(`/orders/${o.id}`)}
        style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: pc ? `${pc}06` : '#FAFAF8', border: `1px solid ${pc ? pc + '25' : '#EDE9E2'}`, borderLeft: `3px solid ${pc || GOLD}20`, borderRadius: 8, cursor: 'pointer', transition: 'all 0.15s', overflow: 'hidden' }}
      ...
      >
        {img
          ? <Image className="order-row-img" src={img} alt="" width={34} height={34} style={{ width: 34, height: 34, objectFit: 'cover', borderRadius: 7, flexShrink: 0, border: '1px solid #E8E0D4' }} onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          : <div className="order-row-img" style={{ width: 34, height: 34, borderRadius: 7, flexShrink: 0, background: '#F5F3EF', border: '1px dashed #D4CEC6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>🖼</div>
        }
        <div className="order-row-text" style={{ minWidth: 0, flex: 1, overflow: 'hidden' }}>
          <div className="order-row-name" style={{ fontSize: 13, fontWeight: 700, color: NAVY, marginBottom: 2, fontFamily: 'Cormorant Garamond, Georgia, serif' }}>{o.poNumber}</div>
          <div className="order-row-sub" style={{ fontSize: 11, color: '#9BA8B5', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
  ...
  const ActionSection = ({ title, subtitle, items, link, showMore, setShowMore }: any) => (
    <div className="dash-action-card" style={{ ...card, padding: '16px 18px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
        <div>
          <h2 style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: 18, fontWeight: 600, color: NAVY, margin: '0 0 2px' }}>{title}</h2>
          {subtitle && <div style={{ fontSize: 11.5, color: '#9BA8B5' }}>{subtitle}</div>}
        </div>
        <a href={link} style={{ fontSize: 12, color: GOLD, fontWeight: 600, textDecoration: 'none', marginTop: 2 }}>View all →</a>
      </div>
      {loading
        ? <div style={{ color: '#9BA8B5', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>Loading…</div>
        : items.length === 0
  ...
  const roleConfigs: Record<string, any> = {
    ...
    FACTORY_VIEWER:  { subtitle: 'Production Queue (Read-Only)', kpis: [{ label: 'VPO Active', value: myVpoTotal, color: '#0891B2', sub: 'in production', link: '/orders?status=VPO_ISSUED' }, { label: 'Manufactured', value: myManufacturedTotal, color: '#8B5CF6', sub: 'done, en route to US', link: '/orders?status=MANUFACTURED' }, { label: 'Priority', value: actions.length, color: '#DC2626', sub: 'need attention', link: '/todos' }] },
  ...
  if (cfg) return (
    <AppLayout title="Dashboard" subtitle={cfg.subtitle} actions={userRole === 'FACTORY_VIEWER' ? undefined : <button onClick={() => router.push('/orders')} style={{ background: NAVY, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 20px', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>+ New Order</button>}>
